...
describe('visit', () => {
  describe('traversal order', () => {
    ...
    it('visits sequence items in order, parent before children', () => {
      const stream = [doc(seq(scalar('a'), scalar('b'), scalar('c')))]
      assert.deepEqual(collect(stream), ['sequence', 'a', 'b', 'c'])
    })

    it('visits each mapping key before its value', () => {
      const stream = [doc(map([scalar('k1'), scalar('v1')], [scalar('k2'), scalar('v2')]))]
      assert.deepEqual(collect(stream), ['mapping', 'k1', 'v1', 'k2', 'v2'])
    })

    it('descends depth-first (pre-order) through nested containers', () => {
      const stream = [doc(map(
        [scalar('a'), seq(scalar('b'), scalar('c'))],
        [scalar('d'), scalar('e')]
      ))]
      assert.deepEqual(collect(stream), ['mapping', 'a', 'sequence', 'b', 'c', 'd', 'e'])
    })

    it('treats an alias as a leaf (no descent)', () => {
      const stream = [doc(seq(scalar('a'), alias('x')))]
      assert.deepEqual(collect(stream), ['sequence', 'a', '*x'])
    })
  })
  ...
  describe('VISIT_SKIP', () => {
    it('does not descend into a skipped container but continues with siblings', () => {
      ...
      const stream = [doc(seq(scalar('a'), skipped, scalar('b')))]
      const seen = collect(stream, node => (node === skipped ? VISIT_SKIP : undefined))
    ...
    it('is a no-op on a leaf', () => {
      const stream = [doc(seq(scalar('a'), scalar('b')))]
      const seen = collect(stream, node => (node.kind === 'scalar' ? VISIT_SKIP : undefined))
  ...
  describe('VISIT_BREAK', () => {
    it('stops the whole traversal immediately', () => {
      const stream = [doc(seq(scalar('a'), scalar('stop'), scalar('c')))]
      const seen = collect(stream, node => (node.value === 'stop' ? VISIT_BREAK : undefined))
    ...
    it('breaking on a mapping key stops before the value', () => {
      const stream = [doc(map([scalar('k'), scalar('v')]))]
      const seen = collect(stream, node => (node.value === 'k' ? VISIT_BREAK : undefined))
      assert.deepEqual(seen, ['mapping', 'k'])
    })

    it('stops across documents', () => {
      const stream = [doc(scalar('stop')), doc(scalar('next'))]
      const seen = collect(stream, node => (node.value === 'stop' ? VISIT_BREAK : undefined))
  ...
  describe('stream level', () => {
    it('walks every document in order', () => {
      const stream = [doc(scalar('a')), doc(seq(scalar('b'))), doc(scalar('c'))]
      assert.deepEqual(collect(stream), ['a', 'sequence', 'b', 'c'])
    })

    it('skips documents with empty contents', () => {
      const stream = [doc(null), doc(scalar('a')), doc(null), doc(scalar('b'))]
      assert.deepEqual(collect(stream), ['a', 'b'])
    })

    it('never calls the visitor for an empty stream', () => {
      let calls = 0
      visit([], () => { calls++ })
